...
UI.registerHelper 'a', ->
  ...
  @route = Luma.Router.getRoute @route
  if "path" of @route
    @href = @route.path
  else @href = '#'
  if @route.header
    if @route.header.heading
      @anchor ?= @route.header.heading
    # Unless already defined the anchor title attr is the route page subtitle.
    if @route.header.subHeading
      @title ?= @route.header.subHeading
  ...
  if @external or @route.external
    ...
    if @styles
      @styles = "#{ @styles } #{ externalLinkClass }"
    else @styles = externalLinkClass
